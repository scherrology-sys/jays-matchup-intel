...
cat("\n=== BASELINE TOURNAMENT ===\n")
# =============================================================================
# 6. BASELINE TOURNAMENT — computed from data, not asserted
# =============================================================================

# Baseline A: average prior wOBA of this lineup (2024-25 combined)
baseline_A <- champion_results |>
  left_join(priors$overall |> select(player_name, wOBA_overall), by="player_name") |>
  summarise(A=round(mean(wOBA_overall, na.rm=TRUE),3)) |>
  pull(A)

# Baseline B: platoon average (LHH vs RHP, RHH vs RHP), park-adjusted
HAND_BASELINES <- c(L=0.307, R=0.322)
baseline_B <- champion_results |>
  left_join(priors$hand |> select(player_name, hand), by="player_name") |>
  mutate(platoon_woba=round(HAND_BASELINES[hand]*PARK_FACTORS[PARK],3)) |>
  summarise(B=round(mean(platoon_woba, na.rm=TRUE),3)) |>
  pull(B)

# Baseline C: team 2026 actual wOBA (computed above)
baseline_C <- team_woba_2026

# Baseline D: pitcher quality proxy
# WR 2025 wOBA allowed = .308 (Baseball Savant). 2026 ERA 2.31 in 11.2 IP — too small.
# Career ERA 4.21 produces .306 via formula, consistent with 2025 observed.
WR_WOBA_ALLOWED_2025 <- 0.308
baseline_D <- round(WR_WOBA_ALLOWED_2025 * PARK_FACTORS[PARK], 3)

# Baseline E: champion model
baseline_E <- round(mean(champion_results$exp_wOBA, na.rm=TRUE), 3)

tournament <- tribble(
  ~baseline, ~label,                                          ~value,
  "A",       "Hitter prior wOBA (2024-25 combined)",          baseline_A,
  "B",       "Platoon baseline, park-adjusted (TOR 0.99)",    baseline_B,
  "C",       "Team 2026 actual wOBA to date",                 baseline_C,
  "D",       "Pitcher proxy (WR 2025 wOBA allowed x 0.99)",   baseline_D,
...
print(tournament)
cat(sprintf("\nChampion: %.3f\n", baseline_E))
cat(sprintf("vs A: %+.3f | vs B: %+.3f | vs C: %+.3f | vs D: %+.3f\n",
    baseline_E-baseline_A, baseline_E-baseline_B,
    baseline_E-baseline_C, baseline_E-baseline_D))
cat("Note: +0.002 over A is within noise. Model value is in lineup ordering, not team aggregate.\n")
