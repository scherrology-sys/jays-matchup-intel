...
# =============================================================================
# L. HITTER IN-SEASON UPDATING  [NEW — April 6, 2026]
# =============================================================================
#
# What this adds
# --------------
# The original framework uses prior-season pitch-type splits for hitters and
# never updates them during the current season. This means the model is blind
# to how pitchers are adapting to Jays hitters as 2026 data accumulates.
#
# This section adds:
#   1. compute_hitter_season_splits() — rolling recency-weighted 2026 splits
#   2. blend_hitter_priors()         — blend prior-season + in-season splits
#   3. detect_hitter_vulnerabilities() — flag pitch types where the hitter is
#                                        significantly under/over their prior
#   4. detect_pitcher_targeting()    — flag when tonight's pitcher throws into
#                                        a documented hitter vulnerability
#
# Blending logic
# --------------
# Both prior-season and in-season splits are weighted by effective sample:
#   eff_prior  = prior-season PA count (large, stable)
#   eff_season = recency-weighted sum of 2026 PAs (small, growing)
#
# The blend fraction for the in-season split:
#   w_season = eff_season / (eff_prior + eff_season)
#
# At 6 games w_season ≈ 0.05–0.15. By July it is 0.40+.
# The prior-season signal dominates early and yields gracefully as 2026 grows.
#
# Half-life for hitter in-season updates: 90 days (faster adaptation than
# pitcher mix, which uses 180 days, because hitter approach adjustments
# happen faster than full repertoire changes).
#
# Vulnerability detection
# -----------------------
# A pitch type is flagged as a vulnerability if the hitter's 2026 wOBA
# against that pitch type is >= VULN_THRESHOLD below their 2025 value
# AND there are at least MIN_SEASON_PA observations in 2026.
#
# Targeting flag
# --------------
# Tonight's pitcher is flagged as targeting a vulnerability if:
#   - The hitter has a documented vulnerability (vuln flag = TRUE)
#   - The pitcher throws that pitch type at above-average rate to that
#     hitter's handedness (>= TARGETING_THRESHOLD × their overall usage)
# This is the bidirectional loop: pitcher adapts to hitter, hitter adapts
# back. The retro scores whether the targeting actually happened.
# =============================================================================

HL_HITTER        <- 90L    # half-life for in-season hitter updating (days)
VULN_THRESHOLD   <- 0.080  # wOBA drop vs prior to flag as vulnerability
VULN_MIN_PA      <- 3L     # min effective in-season PAs to flag
TARGETING_FACTOR <- 1.25   # pitcher throws pitch X at >= 1.25x normal rate = targeting

# Build rolling in-season hitter splits from a list of 2026 game files.
# game_file_list: named list of (game_date -> data_frame) for Jays batting games
# game_date: reference date for recency weighting (typically today's game date)
compute_hitter_season_splits <- function(
  game_file_list,
  game_date,
  half_life_days = HL_HITTER
) {
  ref <- as.Date(game_date)

  all_games <- map_dfr(names(game_file_list), function(gdate) {
    df <- game_file_list[[gdate]]
    if (!"game_date" %in% names(df)) df <- df |> mutate(game_date = gdate)
    df |> mutate(
      game_date_ref = as.Date(gdate),
      age_days = pmax(as.numeric(ref - as.Date(gdate)), 0),
  ...
  # Overall in-season wOBA
  ov_season <- pa |>
    group_by(player_name) |>
    summarise(
      eff_season    = sum(rw, na.rm=TRUE),
      wn_season     = sum(woba_val_w, na.rm=TRUE),
      wd_season     = sum(woba_den_w, na.rm=TRUE),
      wOBA_season   = round(wn_season / pmax(wd_season, 0.01), 3),
      .groups = "drop"
    )

  # Pitch-type in-season splits
  sp_season <- pa |>
    filter(!is.na(pitch_name), pitch_name != "Unknown") |>
    group_by(player_name, pitch_name) |>
    summarise(
      eff_PA_vs     = sum(rw, na.rm=TRUE),
      sp_wn_w       = sum(woba_val_w, na.rm=TRUE),
      sp_wd_w       = sum(woba_den_w, na.rm=TRUE),
      wOBA_season_vs_pitch = round(sp_wn_w / pmax(sp_wd_w, 0.01), 3),
      .groups = "drop"
    )

  list(overall = ov_season, splits = sp_season)
}

# Blend prior-season and in-season hitter priors.
# Returns a list with blended overall wOBA and pitch-type splits per hitter.
blend_hitter_priors <- function(
  prior_season_priors,   # output of build_priors() on prior-season data
  season_splits,         # output of compute_hitter_season_splits()
  prior_season_pa_scale  = 1.0  # multiplier on prior-season PA weight (default: full weight)
) {
  ov_prior   <- prior_season_priors$overall
  sp_prior   <- prior_season_priors$splits
  hd_prior   <- prior_season_priors$hand
  ov_season  <- season_splits$overall
  sp_season  <- season_splits$splits

  # For each hitter: blend overall wOBA
  ov_blended <- ov_prior |>
    left_join(ov_season |> select(player_name, eff_season, wOBA_season),
              by = "player_name") |>
    mutate(
      eff_season   = replace_na(eff_season, 0),
      wOBA_season  = replace_na(wOBA_season, wOBA_overall),
      w_season     = eff_season / (PA_hist * prior_season_pa_scale + eff_season),
      wOBA_blended = round((1 - w_season) * wOBA_overall + w_season * wOBA_season, 3)
    )

  # For each hitter-pitch: blend pitch-type wOBA
  sp_blended <- sp_prior |>
    rename(wOBA_prior = pitch_wOBA) |>
    full_join(
      sp_season |> select(player_name, pitch_name, eff_PA_vs,
                          wOBA_season_vs_pitch),
      by = c("player_name","pitch_name")
    ) |>
    mutate(
      PA_vs         = replace_na(PA_vs, 0),
      eff_PA_vs     = replace_na(eff_PA_vs, 0),
      wOBA_season_vs_pitch = replace_na(wOBA_season_vs_pitch, wOBA_prior),
      wOBA_prior    = replace_na(wOBA_prior, LEAGUE_WOBA),
      w_season_pt   = eff_PA_vs / (PA_vs * prior_season_pa_scale + eff_PA_vs),
      pitch_wOBA    = round((1 - w_season_pt) * wOBA_prior +
                              w_season_pt * wOBA_season_vs_pitch, 3)
    ) |>
    # Retain PA_vs column for MIN_PA threshold in exp_wOBA computation
    mutate(PA_vs = pmax(PA_vs, as.integer(round(eff_PA_vs))))

  list(
    overall = ov_blended |>
      transmute(player_name, PA_hist, wOBA_overall = wOBA_blended),
    splits   = sp_blended |>
      select(player_name, pitch_name, PA_vs, pitch_wOBA),
    hand     = hd_prior,
    ov_detail = ov_blended   # full detail for inspection
  )
}

# Detect pitch-type vulnerabilities for each hitter.
# Returns a data frame with hitter, pitch_name, wOBA_prior, wOBA_season,
# delta, and a vuln_flag boolean.
detect_hitter_vulnerabilities <- function(
  prior_season_priors,
  season_splits,
  vuln_threshold  = VULN_THRESHOLD,
  min_season_pa   = VULN_MIN_PA
) {
  sp_prior  <- prior_season_priors$splits |> rename(wOBA_prior = pitch_wOBA)
  sp_season <- season_splits$splits

  sp_prior |>
    inner_join(
      sp_season |>
        filter(eff_PA_vs >= min_season_pa) |>
        select(player_name, pitch_name, eff_PA_vs, wOBA_season_vs_pitch),
      by = c("player_name","pitch_name")
    ) |>
    mutate(
      delta     = round(wOBA_season_vs_pitch - wOBA_prior, 3),
      vuln_flag = delta <= -vuln_threshold,
      lift_flag = delta >= vuln_threshold
    ) |>
    arrange(player_name, delta)
}

# Detect whether tonight's pitcher is targeting a hitter's documented weakness.
# Returns a data frame with hitter, pitch_name, pitcher_usage, threshold,
# and targeting_flag boolean.
detect_pitcher_targeting <- function(
  vulnerability_df,
  pitcher_mix_lhh,
  pitcher_mix_rhh,
  hitter_hand_df,
  targeting_factor = TARGETING_FACTOR
) {
  # Overall pitcher usage across both handedness splits
  all_pitches <- c(pitcher_mix_lhh, pitcher_mix_rhh)
  overall_usage <- tapply(all_pitches, names(all_pitches), mean)

  vulnerability_df |>
    filter(vuln_flag) |>
    left_join(hitter_hand_df, by = "player_name") |>
    mutate(
      pitcher_usage_vs_hand = map2_dbl(pitch_name, hand, function(p, h) {
        mx <- if (h == "L") pitcher_mix_lhh else pitcher_mix_rhh
        if (p %in% names(mx)) mx[p] else 0
      }),
      overall_pitcher_usage = map_dbl(pitch_name, function(p) {
        if (p %in% names(overall_usage)) overall_usage[p] else 0
      }),
      targeting_flag = pitcher_usage_vs_hand >=
        overall_pitcher_usage * targeting_factor,
      targeting_note = if_else(
        targeting_flag,
        paste0("Pitcher throws ", pitch_name, " at ",
               round(pitcher_usage_vs_hand * 100, 1),
               "% to ", hand, "HH vs ", round(overall_pitcher_usage * 100, 1),
               "% overall — targeting documented weakness"),
        NA_character_
      )
    ) |>
    filter(!is.na(hand)) |>
    select(player_name, hand, pitch_name, delta, eff_PA_vs,
           pitcher_usage_vs_hand, overall_pitcher_usage,
           targeting_flag, targeting_note)
}

# Convenience wrapper: given a list of 2026 game files, compute everything
# and return blended priors + vulnerability + targeting flags ready for
# build_preview().
prepare_hitter_intel <- function(
  game_file_list,      # named list: game_date -> data_frame
  game_date,           # tonight's game date
  prior_season_priors, # from build_priors() on prior year
  pitcher_mix_lhh,
  pitcher_mix_rhh,
  half_life_days     = HL_HITTER,
  vuln_threshold     = VULN_THRESHOLD,
  ...
  season_splits <- compute_hitter_season_splits(
    game_file_list, game_date, half_life_days)

  blended_priors <- blend_hitter_priors(
    prior_season_priors, season_splits)

  vulnerabilities <- detect_hitter_vulnerabilities(
    prior_season_priors, season_splits, vuln_threshold, min_season_pa)

  targeting <- detect_pitcher_targeting(
    vulnerabilities, pitcher_mix_lhh, pitcher_mix_rhh,
    blended_priors$hand, targeting_factor)

  list(
    blended_priors  = blended_priors,
